...
{\large\bfseries Project Website\par}
\vspace{0.2cm}
{\large \url{https://anay-a-joshi.github.io/StochastiQ/}\par}
\vspace{1cm}
...
\vspace{0.2cm}
{\large Professor\ Satyajit Karnik\par}
\vspace{0.5cm}
...
This project was carried out under \emph{Project Idea 6} (independent direction). It is structured as a substantial extension of \emph{Project Idea 1}, whose blueprint we adopted in full and then expanded along several dimensions that we believe were necessary to draw scientifically meaningful conclusions.
...
\begin{table}[!htbp]
\centering
\caption{Calibrated parameter values for AAPL and MSFT (annualized where applicable). The Merton jump intensity $\lambda$ is reported as expected jumps per year. The full calibrated parameter set for all 7 tickers (154 rows) is persisted in \texttt{data/processed/calibrated\_parameters.parquet}.}
\label{tab:calibration}
\small
\begin{tabular}{llrr}
\toprule
Model & Parameter & AAPL & MSFT \\
\midrule
GBM    & Drift $\mu$                    & $+0.298$ & $+0.249$ \\
       & Volatility $\sigma$            & $+0.317$ & $+0.305$ \\
\midrule
Merton & Drift $\mu$                    & $+0.302$ & $+0.253$ \\
       & Diffusion volatility $\sigma$  & $+0.271$ & $+0.260$ \\
       & Jump intensity $\lambda$       & $+3.81$  & $+3.81$  \\
       & Mean log-jump $\mu_J$          & $+0.012$ & $+0.015$ \\
       & Log-jump dispersion $\sigma_J$ & $+0.087$ & $+0.084$ \\
\midrule
CEV    & Drift $\mu$                    & $+0.298$ & $+0.249$ \\
       & Scale $\sigma$                 & $+24.66$ & $+42.87$ \\
       & Elasticity $\beta$             & $+0.100$ & $+0.100$ \\
\midrule
Heston & Drift $\mu$                    & $+0.298$ & $+0.249$ \\
       & Mean-reversion $\kappa$        & $+2.27$  & $+2.17$  \\
       & Long-run variance $\theta$     & $+0.101$ & $+0.094$ \\
       & Vol-of-vol $\xi$               & $+0.900$ & $+0.986$ \\
       & Leverage correlation $\rho$    & $-0.031$ & $-0.066$ \\
       & Initial variance $v_0$         & $+0.027$ & $+0.038$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!htbp]
\centering
\caption{Phase~7 verdict table summarizing the headline findings against pre-registered decision rules. All 95\% confidence intervals computed via $B = 5{,}000$ bootstrap replicates.}
\label{tab:verdict}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrl}
\toprule
Test & Estimate & 95\% CI & $p$-value & Verdict \\
\midrule
Min-max vs.\ EW (Stress)               & $-0.056$    & $[-0.633,\, +0.482]$    & 0.843 & Underpowered \\
Min-max vs.\ EW (Calm)                 & $+0.073$    & $[-0.392,\, +0.531]$    & 0.761 & Underpowered \\
VIX premium (Stress $-$ Calm)          & $+15.74$    & $[+13.77,\, +17.86]$    & ---   & \textbf{Validated (large)} \\
HMM vs.\ Realized-Vol $\kappa$         & $+0.235$    & $[+0.202,\, +0.271]$    & ---   & Weak agreement \\
Min-max stress CVaR vs.\ EW            & $+51.5$ bps & $[-29.2,\, +127.5]$ bps & ---   & \textbf{Better tail risk (point)} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
StochastiQ/
|-- data/
|   |-- processed/                    # Parquet result files (gitignored)
|   |-- raw/                          # Raw price downloads (gitignored)
|   `-- README.md
...
|   |-- 04_monte_carlo_robust_portfolio.ipynb
|   |-- 05_options_overlay.ipynb
|   |-- 06_out_of_sample_validation.ipynb
|   |-- 07_regime_analysis.ipynb
|   `-- 08_final_results.ipynb
|-- reports/
|   |-- figures/                      # 35 generated PNGs across all phases
|   |-- tables/
|   |-- build_report.sh               # One-command compile script
|   |-- references.bib                # Bibliography
|   |-- StochastiQ_Final_Report.tex   # This document
|   `-- StochastiQ_Final_Report.pdf   # Compiled output
|-- src/                              # Production source code
|   |-- config.py                     # Global constants (rf, trading days, seed=42)
|   |-- analysis/
|   |-- calibration/
|   |   `-- calibrators.py            # MLE / MoM fitting routines
|   |-- data/
|   |   `-- loaders.py                # yfinance ingestion, return computation
...
|   |   |-- black_scholes.py          # Black-Scholes-Merton pricing + Greeks
...
|   |-- simulation/
|   |   `-- monte_carlo.py            # Cholesky-correlated path generation
|   |-- utils/
|   `-- validation/
|       |-- bootstrap.py              # Politis-Romano stationary bootstrap
|       `-- oos.py                    # Out-of-sample evaluation utilities
|-- tests/                            # Unit tests
|   `-- test_models.py
|-- LICENSE                           # MIT
|-- README.md
...
All authors/team-members reviewed and approved the final report.
